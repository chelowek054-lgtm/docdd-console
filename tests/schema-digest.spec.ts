...
describe('выжимка из схемы', () => {
  ...
  it('несёт описание поля из схемы, а не только имя и «обязательное»', () => {
    // Без этого модель не видела разницы между `id` (может быть логическим
    // именем — «gastrograf.domain.nutrition») и `path` (всегда буквальный
    // файл, которым карта позволяет открыть узел кликом, docs/04-ui.md) — оба
    // читались одинаково «путь», и клик открывал не то или ничего.
    const digest = schemaDigest(codemapSchema, 'docdd-codemap', REFS);
    expect(digest).toContain('`path` — Файл, где модуль определён — путь от корня проекта.');
  });
