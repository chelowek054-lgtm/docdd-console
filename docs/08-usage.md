...
Заодно в корне проекта появится **`CLAUDE.md`** — правила для модели, которая
будет с этим проектом работать: что документ ведёт код, где лежат записи
процесса, чего трогать нельзя. Без него модель, запущенная в папке проекта,
про DocDD не знает ничего и работает как обычно — то есть мимо правил.

Файл уже есть — приложение его не трогает: это ваши правила, и заводить формат
не значит их переписывать.
...
| Что ждёт моего подтверждения? | Документы — неподтверждённое наверху; Карты — черновики над картиной |
| Как идёт этап? | Работа → Фазы: полоса готовности и что мешает закрыть |
