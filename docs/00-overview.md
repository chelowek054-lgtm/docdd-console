# Назначение и границы

## Задача

Правило «документ ведёт код» легко объявить и трудно соблюдать: связь между
требованием, документом, задачей и проверкой держится на внимательности того,
кто её помнит. Приложение делает эту связь машиночитаемой и показывает, где она
рвётся.

Три вопроса, на которые оно отвечает:

1. **В каком состоянии работа?** Сколько задач в каждом статусе, что зависло,
   какая фаза идёт.
2. **Что нарушает процесс?** Задача в работе с неподтверждённым документом,
   требование без единой проверки, закрытая задача с непройденным тестом.
3. **Что можно взять в работу прямо сейчас?** Задачи, у которых всё готово.

## Кто пользуется

Один человек на своей машине — но не обязательно программист. Записи процесса
(требования, задачи, карты устройства) ведёт архитектор или разработчик;
знания о предметной области — доменный специалист, через входящее
([10-inbox.md](10-inbox.md)), без markdown и без git. Прав и ролей нет: роль
в файле это подпись, а не доступ ([06-phases.md](06-phases.md)).

## Что приложение делает

- Читает `docs/development` выбранного проекта и строит граф записей.
- Проверяет схемы, ссылочную целостность и правила процесса.
- Показывает состояние работы: списки, карточки, граф, нарушения.
- Меняет статусы и пишет строку в журнал записи.
- Создаёт новые записи из шаблонов.
- Рисует диаграммы mermaid из документов.
- Читает отчёты прогонов тестов и связывает их с требованиями.

## Что приложение не делает — и это решения, а не «пока не успели»

| Не делает | Почему |
|---|---|
| Не заводит базу данных | Источник правды — файлы в репозитории проекта. Вторая копия начнёт расходиться в тот же день ([ADR-0001](adr/0001-files-are-the-truth.md)) |
| Не даёт модели переписывать текст документов | Человек через интерфейс — может; правка текста — действие человека, а не модели ([ADR-0011](adr/0011-body-editing.md)) |
| Не переписывает историю git | Заводит ветку задачи, коммитит принятое и сливает перемоткой — и только это ([ADR-0009](adr/0009-work-through-console.md)) |
| Не запускает сборки и тесты | Отчёт о прогоне кладёт сборка проекта, приложение его читает |
| Не сливает без человека | Задачу выполняет модель, но дифф разбирает и принимает человек — ни при каких настройках иначе |
| Не решает за человека | Модель можно спросить кнопкой — через Claude Code на этой же машине, — но ответ ничего не меняет сам: подтверждает человек ([ADR-0008](adr/0008-llm-through-claude-code.md)) |
| Не хранит обсуждения и комментарии | Для этого есть git и мессенджер |
| Не считает оценки и трудозатраты | Они не проверяются фактом и быстро врут |
| Не смотрит наружу | Локальный инструмент без аутентификации и удалённого доступа |

## Границы

```mermaid
flowchart LR
    user(("Человек"))
    subgraph app["DocDD Console"]
        ui["Интерфейс"]
        srv["Серверная часть Nitro"]
    end
    fs[("Файлы проекта<br/>docs/development")]
    code[("Исходники проекта")]
    reports[("Отчёты прогонов")]

    user --> ui --> srv
    srv <--> fs
    srv --> code
    srv --> reports
```

Приложение читает исходники проекта для двух целей: проверить, что ссылки из
документов ведут в существующие места, и сверить свидетельства карт — что
объявленная строка в файле действительно есть. Синтаксис оно не разбирает и
ничего в исходниках не меняет ([adr/0007-maps-lead-code.md](adr/0007-maps-lead-code.md)).

## Успех

Инструмент считается удавшимся, если им пользуются не ради отчётности, а потому,
что без него непонятно, что брать в работу. Признак обратного — папка
`docs/development`, которую правят руками в обход приложения, потому что через
него медленнее.
