...
flowchart LR
    ...
    f6 --> f7["7. Карты проекта"]
    f7 --> f8["8. Выполнение через клиент"]
```
...
языковым моделям сам ([ADR-0008](adr/0008-llm-through-claude-code.md)): запрос
можно отправить кнопкой, но ответ остаётся предложением. Его дело — показать,
...
|---|---|
| `0` | Ошибок нет. Предупреждения могли быть — они на код возврата не влияют |
| `1` | Есть хотя бы одна **ошибка**: правило, которое валит сборку, обязано быть правилом, а не мнением ([05-validation.md](05-validation.md)) |
| `2` | Проект не прочитан: нет манифеста, чужое поколение контракта, неверный вызов. Это не «нашли нарушение», и путать эти два случая в сборке нельзя |

- Вывод в двух видах: для человека и `--json` для сборки.
- Ничего не пишет: ни файлов, ни кэша. Кэш индекса — забота приложения, а
  проверка в чужом конвейере не должна оставлять следов в чужом репозитории.
...
## Фаза 7. Карты проекта

Три структуры, которых нет в записях процесса: кодовая база, потоки данных,
пользовательские пути ([07-maps.md](07-maps.md)). Карту строит модель снаружи,
приложение её проверяет и по ней блокирует работу
([adr/0007-maps-lead-code.md](adr/0007-maps-lead-code.md)).

- Тип записи `map`, связь `affects`, поле `change` у задачи.
- Разбор блоков `docdd-codemap`, `docdd-dataflow`, `docdd-userflow` и проверка
  по трём схемам.
- Сверка свидетельств с файлами: путь, строка, фрагмент.
- Сверка на закрытой задаче: добавленное есть, убранное отсутствует.
- Складывание подтверждённых карт в общую картину проекта.
- Три вида на экране и шаблоны промптов в репозитории.

**Готово, когда:** карта, полученная от модели, проходит проверку свидетельств;
подделанное ребро ловится; задача с `change: feature` не уходит в работу без
подтверждённой карты; по общей картине видно, что на что опирается, откуда
берутся данные и какими путями ходит человек.

## Фаза 8. Выполнение через клиент

Задача проходит путь от подтверждённого намерения до слитого кода, не покидая
инструмента ([09-execution.md](09-execution.md)). Клиент при этом остаётся
воротами, а не становится проходным двором
([adr/0009-work-through-console.md](adr/0009-work-through-console.md)).

- Ветка и рабочее дерево на задачу; ваш каталог не трогается.
- Запрос из задачи, её требования, документов, карты изменения и сжатой карты
  проекта.
- Дифф на экране; принять, отправить на доработку или отклонить.
- Слияние перемоткой по подтверждению; `main` ушёл вперёд — отказ с причиной.
- Отклонение слияния, если модель тронула `docs/development`.
- Журнал захода в записи и два кода нарушений про незавершённый разбор.

**Готово, когда:** задача в `ready` доходит до слияния кнопками, доработка идёт
в той же ветке, а попытка модели поправить запись процесса не сливается.
