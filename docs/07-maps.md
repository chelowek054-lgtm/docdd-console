...
сразу и целиком. Поэтому функциональная карта — единственная, что рисуется
не диаграммой, а сворачиваемым деревом (`app/components/FunctionalTree.vue`):
подпункты прячутся под родителем, глубже двух уровней — по умолчанию
свёрнуто, названия — обычным текстом без иконок связей и стрелок.
