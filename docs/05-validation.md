...
Разбор трогает не всё подряд. Файл в `docs/development`, который не начинается
с `---`, — не запись, а справочный текст рядом: README папки, вендоренная
копия документации формата, что угодно ещё. Такой файл не разбирается и не
попадает в нарушения вовсе — ему нечего чинить, потому что он не пытался быть
записью. Начал с `---`, но дальше не разобрался — это уже `parse_failed`:
такой файл заявил о себе как о записи и не смог.
...
| Код | Уровень | Когда |
|---|---|---|
| `parse_failed` | error | Файл начинается с `---`, но дальше не разбирается: битый YAML, front matter — не набор полей |
...
`schema_invalid` молчит на записи в статусе `dropped`, `superseded` или
`rejected`: содержание отставленной записи не читается как текущее, и
недостающее поле — не дело, которое кому-то доделывать. Та же запись,
дошедшая до отставленного статуса со сломанной схемой (заводили вручную, в
обход приложения), остаётся видимой через `supersedes` у преемника — просто
не соревнуется за внимание с тем, что действительно ждёт правки. То же самое
у `map_invalid` — ниже.
...
| Код | Уровень | Когда |
|---|---|---|
| `map_invalid` | error | Блок карты не разбирается или не проходит свою схему; молчит на `dropped`/`superseded`/`rejected` — см. выше |
