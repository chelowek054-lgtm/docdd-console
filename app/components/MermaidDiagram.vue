...
async function draw() {
  ...
  try {
    ...
    mermaid.initialize({
      ...
      // Пределы mermaid по умолчанию (50 000 знаков, 500 рёбер) — защита от
      // чужого текста на странице. Здесь текст наш, из подтверждённых карт, и
      // общая картина проекта в сотню модулей их перерастает: вместо графа
      // mermaid рисует плашку «Maximum text size in diagram exceeded», не
      // бросая ошибки. Плотный граф оправдан зумом (docs/04-ui.md, «Карты»).
